...
- **In-memory TTL cache over a durable snapshot store.** Simple and fast, but a cold start loses the snapshot — after which degradation is honest-but-empty rather than stale-but-useful. Accepted and documented; the durable store is first in the improvements list.
